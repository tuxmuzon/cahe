...
void RR_Cash::UpdateVidCheka(int Vid){
    ...
    } else QMessageBox::warning(0,"Ошибка смены типа документа", "Невозможно изменить типа документа,список товаров не пуст");
}
...
void RR_Cash::on_tab_2_clicked(){
    ...
    QString queryString;
    if (barcode.length()>5) {
        queryString="SELECT name, units, price, alc, excise, code, barcode, group_print, volume, user_code FROM goods, barcode WHERE barcode.goods_id=goods.code and barcode.barcode="+barcode;
    } else {
        queryString="SELECT name, units, price, alc, excise, code, barcode, group_print, volume, user_code  FROM goods, barcode WHERE barcode.goods_id=goods.code and user_code="+barcode;
    }
    ...
    if(query.value(0).toString()!="") {
        ...
        if (!find_repeat){
            ...
            ui->table_tovar->setItem(0,10,new QTableWidgetItem(query.value(9).toString()));
